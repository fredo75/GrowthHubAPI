...
require 'simple_hubspot'
  ...
  def new
   ...
   url = URI("https://phantombuster.com/api/v1/agent/31385/output?&key=PuzLoIgb0mCaokfWEc51aRpqQvm1nuab")
    ...
    @response = http.request(request)
    @phantombuster_callback_json = JSON.parse(@response.read_body)
    @phantombuster_callback = @phantombuster_callback_json["data"]["resultObject"][0]["firstName"]
    # @contactshub = SimpleHubspot::Contact.create_or_update "#{@phantombuster_callback_json["data"]["resultObject"][0]["firstName"]}@azerty.com", { firstname: "#{@phantombuster_callback_json["data"]["resultObject"][0]["firstName"]}", lastname: "#{@phantombuster_callback_json["data"]["resultObject"][0]["lastName"]}" }
    ...
    # @contactshub = SimpleHubspot::Contact.create_or_update "#{@phantombuster_callback_parse_json[0]["firstName"]}@azerty.com", { firstname: "#{@phantombuster_callback_parse_json[0]["firstName"]}", lastname: "#{@phantombuster_callback_parse_json[0]["lastName"]}" }
  ...
  def create
    ...
    # @contactshub = SimpleHubspot::Contact.create_or_update "email@email.com", { firstname: "James", lastname: "Cameron", company: 'USA' }
